	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		case "list": {
			if (session != null) {
				String login = (String) session.getAttribute("nombre_usuario");
				if (login != null) {
					DtListaReproduccion[] listasPrivadasSesion = interfazUsuario.infoLDRdeUsuario(null, login,
							Privacidad.PRIVADO);
		...
		case "crearLDR": {
			...
			if (session != null && session.getAttribute("nombre_usuario") != null) {
				...
				if (request.getParameter("grupoPrivacidad").equals("Publico")) {
					...
					if (!interfazUsuario.memberListaReproduccionPropia(nombreUsuario, encodedLista)) {
